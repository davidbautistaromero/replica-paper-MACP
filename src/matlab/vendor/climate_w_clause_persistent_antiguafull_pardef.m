...
%     p_g_choice_guess = p_bar * ones(N_x,N_b_g,N_b_g);
    ...
    v_pardef_guess = zeros(N_x,N_b_g);
...
while diff_v > tol_v && iter_v<maxiter_v && sum_v>1
    ...
    e_v_rel_guess = P_x*v_pardef_guess;
...
%     end
    ...
    v_good_guess_new_pardef = eulgam*ev_rho + ...
    ...
    v_guess_new_pardef = eulgam*ev_rho + v_good_guess_new_pardef + ...
        ev_rho*log(1+exp(1/ev_rho*(-v_good_guess_new_pardef+repmat(v_bad_guess_new,[1,N_b_g]))));   
    
    %Value function in default
    v_guess_new_flip = eulgam*ev_rho + repmat(v_bad_guess_new,[1,N_b_g]) + ...
        ev_rho*log(1+exp(1/ev_rho*(+v_good_guess_new-repmat(v_bad_guess_new,[1,N_b_g]))));
    
    v_guess_new_flip_pardef = eulgam*ev_rho + repmat(v_bad_guess_new,[1,N_b_g]) + ...
        ev_rho*log(1+exp(1/ev_rho*(+v_good_guess_new_pardef-repmat(v_bad_guess_new,[1,N_b_g]))));   
...
%     clear interm
    def_new_pardef = 1-(exp(ev_rho_def*(v_bad_guess_new-v_good_guess_new_pardef))+1).^-1;
    ...
    v_guess_new_pardef(def_new_pardef>.999)=v_guess_new_flip_pardef(def_new_pardef>.999);   
    
    v_guess_new(dummy_rel==1) = v_guess_new_pardef(dummy_rel==1);
    v_guess_new_pardef(dummy_rel==0) = v_guess_new(dummy_rel==0);
...
%     v_guess_new_rel(dummy_rel==1) = v_guess_new(dummy_rel==1);
    ...
    v_pardef_guess = damp_v*v_guess_new_pardef + (1-damp_v)*v_pardef_old;
...
e_defrel = P_x*((1-def_only_pf).*(1-pardef_only_pf).*(1+(1-delta).*q_g_pf_zeros)...
   +(1-def_only_pf).*pardef_only_pf.*share.*(1+(1-delta).*q_g_pf_zeros));



e_defrel2 = P_x*((1-def_only_pf).*(1-pardef_only_pf).*(1+(1-delta).*q_g_pf_zeros)...
   +(1-def_only_pf).*pardef_only_pf.*share.*(1+(1-delta).*qrf_lt));
...
b_g_mean_baseline = lg.b_g_mean_counter(:,counter);
mass_acc_baseline  = lg.mass_acc_counter(:,counter);
c_sim_baseline  = lg.cons_sim(:,counter);
...
mass_only_rel(1,1)=1;
mass_only_def(1,1)=1;
pardef_only_pf(def_only_pf>0.999)=0;    
for t_sim = 1:T_sim1
      %Neither default nor debt relief
     if  mass_only_def(t_sim,1)>0.99 && floor((i_x_sim(t_sim,1)-1)./N_h)==(i_x_sim(t_sim,1)-1)./N_h
         prova(t_sim,1)=1;
    mass_only_def(t_sim+1,1) = sum(sum(dist_sim(:,:,t_sim).*(1-def_only_pf).*(1-pardef_only_pf))); %def_pf already includes reliefs
    ...
    dist_sim(i_x_sim(t_sim+1,1),:,t_sim+1) = sum(sum(repmat(dist_sim(:,:,t_sim).*...
        (1-def_only_pf).*(1-pardef_only_pf),[1,1,N_b_g]).*prob_choice,1) ,2);
    ...
    V_g_mean(t_sim) = sum(sum(v_guess.*dist_sim(:,:,t_sim)))./sum(sum(dist_sim(:,:,t_sim)));
     ...
     elseif  mass_only_def(t_sim,1)>0.99 && floor((i_x_sim(t_sim,1)-1)./N_h)~=(i_x_sim(t_sim,1)-1)./N_h
         prova(t_sim,1)=2;
    mass_only_def(t_sim+1,1) = sum(sum(dist_sim(:,:,t_sim).*(1-def_only_pf).*pardef_only_pf)); %def_pf already includes reliefs
       
    % Evolution of distribution of countries, given probabilistic decision
    % rule and exogenous income shocks
    dist_sim(i_x_sim(t_sim+1,1),:,t_sim+1) = sum(sum(repmat(dist_sim(:,:,t_sim).*...
        (1-def_only_pf).*pardef_only_pf,[1,1,N_b_g]).*prob_choice_pardef,1)  ,2);
    ...
    q_g_mean2(t_sim) = sum(sum(q_g_pf2.*dist_sim(:,:,t_sim)))./sum(sum(dist_sim(:,:,t_sim)));
    b_g_mean(t_sim) = sum(sum(repmat(b_g_vec,[N_x,1]).*dist_sim(:,:,t_sim)))./sum(sum(dist_sim(:,:,t_sim)));
    V_g_mean(t_sim) = sum(sum(v_pardef_guess.*dist_sim(:,:,t_sim)))./sum(sum(dist_sim(:,:,t_sim)));
...
b_g_mean_counter(:,counter,counter_share) = b_g_mean; %Save the path of debt
...
spread_sim_counter(:,counter,counter_share)=spread_sim;
spread_sim_counter2(:,counter,counter_share)=spread_sim2;
...
cons_sim(:,counter,counter_share)=c_sim;

%Statistics
meanBY_sim(counter,counter_share)= nanmean(B_g_sim(spread_sim<1e+4));
meanBY_sim_market(counter,counter_share)= nanmean(q_g_mean(spread_sim<1e+4).*b_g_mean(spread_sim<1e+4));
meanspread_sim(counter,counter_share)= nanmean(spread_sim(spread_sim<1e+4));
meanspread_sim2(counter,counter_share)= nanmean(spread_sim2(spread_sim2<1e+4));
stdspread_sim(counter,counter_share) = std(spread_sim(spread_sim<1e+4),0,1,'omitnan');
stdspread_sim2(counter,counter_share) = std(spread_sim2(spread_sim2<1e+4),0,1,'omitnan');
hur_freq_sim(counter,counter_share)  = size(h_sim(h_sim<1),1)./size(h_sim,1);
...
def_freq_sim(counter,counter_share)  = mean(def_sim);
def_hur_freq_sim(counter,counter_share)  = nanmean(def_sim(h_sim<1));

%GDP Loss|Hurricane
gdp_sim =y_sim.*h_sim;
gdp_sim(isnan(c_sim1)==1)=gdp_mean;
gdp_g = (gdp_sim(2:end,1)-gdp_sim(1:end-1))./gdp_sim(1:end-1);
gdp_g_h_sim(counter,counter_share) = mean(gdp_g(h_sim(2:end,1)<1));


%Spread Increase|Hurricane
spread_g = (spread_sim(2:end,1)-spread_sim(1:end-1))./spread_sim(1:end-1);
spread_g_h_sim(counter,counter_share) = nanmedian(spread_g(h_sim(2:end,1)<1));
...
conequivwc_V_counter(counter,counter_share)  = nanmean(nanmean(prova2));

%%
clearvars -except zz jj string_num meanBY_sim meanBY_sim_market meanspread_sim meanspread_sim2...
    stdspread_sim stdspread_sim2 counter1 counter2 gdp_g_h_sim spread_g_h_sim b_g_mean_counter...
    cons_sim spread_sim_counter spread_sim_counter2 def_freq_sim def_hur_freq_sim hur_freq_sim string_num...
    conequivwc_V_counter conequivwc_W_counter counter_share share

  

save('climate_w_clause_persistent_antiguafull_pardef.mat')
