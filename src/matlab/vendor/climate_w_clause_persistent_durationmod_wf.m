...
wc_par_asymm   = .75;%.8;
delta = share.*0.0824+(1-share).*0.0997;% (8.2 years maturity)
...
delta = share.*0.0442+(1-share).*0.0532;
...
delta = share.*0.0467+(1-share).*0.0489;
sigma_eh = .028;%.006;         % Standard deviation of hurricane
...
delta = share.*0.1731+(1-share).*0.1781;% 
...
delta = share.*0.0612+(1-share).*0.0673;%
...
delta = share.*0.1639+(1-share).*0.1694;%
...
delta = share.*0.0564+(1-share).*0.0706;% (8.2 years maturity)
...
% choices = kron(b_g_vec,ones(N_b_g,1));
% states = b_g_vec'*ones(1,N_b_g);
% sumcs = (1-share).*states+choices;
% clear index
% for bibi=1:N_b_g
%     for bibi1=1:N_b_g
%         [~,index(bibi,bibi1)] = min((sumcs(bibi,bibi1)*ones(1,N_b_g)-b_g_vec).^2);
%     end
% end
% 
% index = repmat(reshape(index,1,N_b_g,N_b_g),N_x,1,1);                                                                                                                                                                                                             
...
while diff_v > tol_v && iter_v<maxiter_v && sum_v>1
    ...
    e_v_rel_guess_3grid_int = permute(repmat(e_v_rel_guess,[1 1 N_b_g]),[1 3 2]);   
    e_v_rel_guess_3grid = e_v_rel_guess_3grid_int;   
    ...
    e_v_rel_guess_rn = P_x*v_guess_rn;
    e_v_rel_guess_3grid_int_rn = permute(repmat(e_v_rel_guess_rn,[1 1 N_b_g]),[1 3 2]);   
    e_v_rel_guess_3grid_rn = e_v_rel_guess_3grid_int_rn;      
...
%         (permute(repmat(b_choice_2grid,[1,1,N_b_g]),[1 3 2])-share.*(1-delta).*b_state_grid-(1-share).*((1-delta)+delta.*(1+mu_r)).*b_state_grid);
% 
    borr_rev_choice_3grid_rel = permute(repmat(q_g,[1,1,N_b_g]),[1 3 2]).*...
   ...
   % Consumption under hurricane relief
    ...
    util_rel_choice_rn = cons_rel_choice;    
    ...
    borrower_maximand_rel_rn = util_rel_choice_rn + beta*e_v_rel_guess_3grid_rn;
...
v_good_guess_noev_rel_rn=zeros(size(v_good_guess_noev_rel,1),size(v_good_guess_noev_rel,2));
    for cicci=1:N_x
        for cicci1 = 1:N_b_g
    v_good_guess_noev_rn(cicci,cicci1) = borrower_maximand_rn(cicci,cicci1,i_b_max_new_noev(cicci,cicci1));
    v_good_guess_noev_rel_rn(cicci,cicci1) = borrower_maximand_rel_rn(cicci,cicci1,i_b_max_new_noev_rel(cicci,cicci1));
        end
    end
    ...
    prob_choice_rel = exp((borrower_maximand_rel-repmat(v_good_guess_noev_rel,[1,1,N_b_g]))/ev_rho)...
        ./repmat(sum(exp((borrower_maximand_rel-repmat(v_good_guess_noev_rel,[1,1,N_b_g]))/ev_rho),3),1,1,N_b_g);
    ...
    v_good_guess_new_rel_rn = eulgam*ev_rho + ...
        ...
        ev_rho*log(sum(exp(1/ev_rho*(borrower_maximand_rel_rn-norma_explog_rel_rn)),3));
...
%     clear interm
    def_new_rel = 1-(exp(ev_rho_def*(repmat(v_bad_guess_new,[1,N_b_g])-v_good_guess_new_rel))+1).^-1;
   def_new_rel(dummy_rel==0)=0;
    ...
    v_guess_new_rel_rn(def_new_rel>.999)=v_guess_new_flip_rel_rn(def_new_rel>.999);   
...
%    +(1-def_only_pf).*rel_only_pf.*(share.*(1+(1-delta).*q_g_rel_pf) +(1-share).*((1-delta)+delta.*(1+mu_r)).*q_g_rel_pf));
...
e_defrel_re = P_x*((1-def_only_pf).*(1-rel_only_pf).*(1+(1-delta).*q_g_rel_rel_pf)...
   +(1-def_new_rel).*rel_only_pf.*(share.*(1+(1-delta).*q_g_rel_rel_pf)+(1-share).*(1+mu_r).*q_g_rel_rel_pf));
...
e_defrel2 = P_x*((1-def_only_pf).*(1-rel_only_pf).*(1+(1-delta).*q_g_pf2)...
   +(1-def_only_pf).*rel_only_pf.*((1+(1-delta).*q_g_pf2)));
...
for t_sim = 1:T_sim1
    ...
    def_mean(t_sim) = sum(sum(def_new.*dist_sim(:,:,t_sim)))./sum(sum(dist_sim(:,:,t_sim)));
     ...
     elseif  mass_only_def(t_sim,1)>0.99 && floor((i_x_sim(t_sim,1)-1)./N_h)~=(i_x_sim(t_sim,1)-1)./N_h
         prova(t_sim,1)=2;
     mass_only_def(t_sim+1,1) = sum(sum(dist_sim(:,:,t_sim).*(1-def_new_rel))); %def_pf already includes reliefs
       
    % Evolution of distribution of countries, given probabilistic decision
    % rule and exogenous income shocks
    dist_sim(i_x_sim(t_sim+1,1),:,t_sim+1) = sum(sum(repmat(dist_sim(:,:,t_sim).*...
        (1-def_new_rel),[1,1,N_b_g]).*prob_choice_rel,1)  ,2);
    ...
    def_mean(t_sim) = sum(sum(def_new_rel.*dist_sim(:,:,t_sim)))./sum(sum(dist_sim(:,:,t_sim)));
...
clearvars -except zz jj string_num meanBY_sim meanBY_sim_market meanspread_sim medianspread_sim stdspread_sim...
    ...
    conequivwc_V_counter1_rn  def_freq_sim1 def_hur_freq_sim1 

save('climate_w_clause_persistent_durationmod_wf.mat')
...
save('climate_w_clause_persistent_wf.mat')
