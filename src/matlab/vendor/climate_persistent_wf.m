...
lambda   = 1/3;   % Dias 0.33

def_cost_asymm = 1; % = 1 if default cost a la arellano, 0 if aguiar gopinath,2 Eygnour

wc_par_symm    = 0.98;        % Welfare cost of def't: Fraction of output available in default state. Not piecewise.
cost2f = 0.20;
cost1f = -0.18;
...
N_y     = 63; % Number of y-gridpoints
N_h     = 20; % Number of h-gridpoints
...
redem_sim_counter(:,counter)=redem_sim;
...
clearvars -except zz jj string_num meanBY_sim meanBY_sim_market meanspread_sim medianspread_sim stdspread_sim...
    ...
    r_g_mean_counter q_g_mean_counter share_counter gdp_mean_store

save('climate_persistent_wf.mat')
save('gdp_mean_storemat','gdp_mean_store')

 end

 clearvars -except meanBY_sim meanBY_sim_market meanspread_sim medianspread_sim...
    gdp_g_h_sim  def_freq_sim  hur_freq_sim     
