...
while diff_v > tol_v && iter_v<maxiter_v && sum_v>1
    ...
    borr_rev_choice_3grid_pardef = permute(repmat(q_g,[1,1,N_b_g]),[1 3 2]).*...
        (permute(repmat(b_choice_2grid,[1,1,N_b_g]),[1 3 2])-share.*(1-delta).*b_state_grid);
   ...
   % Consumption under hurricane relief
    cons_pardef_choice = h_state_grid.*y_state_grid- share.*b_state_grid+borr_rev_choice_3grid_pardef;
    cons_pardef_choice(cons_pardef_choice<eps) = eps;

    
    % Period-utility implied by state and choice for b'. Dim: z,b,b'
    util_choice = 1/(1-gamma_c)*cons_choice.^(1-gamma_c);
    util_pardef_choice = 1/(1-gamma_c)*cons_pardef_choice.^(1-gamma_c);
    util_choice_rn = cons_choice;
    util_pardef_choice_rn = cons_pardef_choice;    
        
    % Form maximand in borrowing choice
    borrower_maximand = util_choice + beta*e_v_guess_3grid;
    borrower_maximand_pardef = util_pardef_choice + beta*e_v_guess_3grid;
    borrower_maximand_rn = util_choice_rn + beta*e_v_guess_3grid_rn;
    borrower_maximand_pardef_rn = util_pardef_choice_rn + beta*e_v_guess_3grid_rn;
     % Choice for borrowing and update value function

     [v_good_guess_noev, i_b_max_new_noev] = max(borrower_maximand,[],3);
     [v_good_guess_noev_pardef, i_b_max_new_noev_rel] = max(borrower_maximand_pardef,[],3);
     
    v_good_guess_noev_rn=zeros(size(v_good_guess_noev,1),size(v_good_guess_noev,2));
    v_good_guess_noev_rel_rn=zeros(size(v_good_guess_noev_pardef,1),size(v_good_guess_noev_pardef,2));
        for cicci=1:N_x
            for cicci1 = 1:N_b_g
        v_good_guess_noev_rn(cicci,cicci1) = borrower_maximand_rn(cicci,cicci1,i_b_max_new_noev(cicci,cicci1));
        v_good_guess_noev_rel_rn(cicci,cicci1) = borrower_maximand_pardef_rn(cicci,cicci1,i_b_max_new_noev_rel(cicci,cicci1));
            end
        end
    ...
    prob_choice_pardef = exp((borrower_maximand_pardef-repmat(v_good_guess_noev_pardef,[1,1,N_b_g]))/ev_rho)...
        ./sum(exp((borrower_maximand_pardef-repmat(v_good_guess_noev_pardef,[1,1,N_b_g]))/ev_rho),3);
    
    norma_explog = repmat(v_good_guess_noev,[1,1,N_b_g]);
    norma_explog_pardef = repmat(v_good_guess_noev_pardef,[1,1,N_b_g]);
    norma_explog_rn = repmat(v_good_guess_noev_rn,[1,1,N_b_g]);
    norma_explog_rel_rn = repmat(v_good_guess_noev_rel_rn,[1,1,N_b_g]);
    ...
    v_good_guess_new_rel = eulgam*ev_rho + ...
        v_good_guess_noev_pardef + ...
        ev_rho*log(sum(exp(1/ev_rho*(borrower_maximand_pardef-norma_explog_pardef)),3));

    v_good_guess_new_rn = eulgam*ev_rho + ...
        v_good_guess_noev_rn + ...
        ev_rho*log(sum(exp(1/ev_rho*(borrower_maximand_rn-norma_explog_rn)),3));
  
    v_good_guess_new_rel_rn = eulgam*ev_rho + ...
        v_good_guess_noev_rel_rn + ...
        ev_rho*log(sum(exp(1/ev_rho*(borrower_maximand_pardef_rn-norma_explog_rel_rn)),3));
    ...
    v_guess_new_rel_rn = eulgam*ev_rho + v_good_guess_new_rel_rn + ...
        ev_rho*log(1+exp(1/ev_rho*(-v_good_guess_new_rel_rn+repmat(v_bad_guess_new_rn,[1,N_b_g]))));   
    ...
    v_guess_new_flip_rel_rn = eulgam*ev_rho + repmat(v_bad_guess_new_rn,[1,N_b_g]) + ...
        ev_rho*log(1+exp(1/ev_rho*(+v_good_guess_new_rel_rn-repmat(v_bad_guess_new_rn,[1,N_b_g]))));      
...
%     interm =  (1-(exp(ev_rho_def*(v_bad_guess_new-v_good_guess_new_rel))+1).^-1);
%     def_new(dummy_rel==1)=interm(dummy_rel==1);
%     clear interm
    def_new_pardef = 1-(exp(ev_rho_def*(v_bad_guess_new-v_good_guess_new_rel))+1).^-1;
    def_new_pardef(dummy_rel==0)=0;
    
    
    %Adjust also the rel value function
  
    v_guess_new(def_new>.999)=v_guess_new_flip(def_new>.999);    
    v_guess_new_rel(def_new_pardef>.999)=v_guess_new_flip_rel(def_new_pardef>.999);   
    ...
    v_guess_new_rel_rn(def_new_pardef>.999)=v_guess_new_flip_rel_rn(def_new_pardef>.999);   
    
    v_guess_new_rn(dummy_rel==1) = v_guess_new_rel_rn(dummy_rel==1);
    v_guess_new_rel_rn(dummy_rel==0) = v_guess_new_rn(dummy_rel==0);
    def_new = def_new+def_new_pardef;
...
%     v_guess_new_rel(dummy_rel==1) = v_guess_new(dummy_rel==1);
    ...
    v_guess_new_rel_rn(v_guess_new_rel_rn==Inf) = v_bad_guess_rn_repmat(v_guess_new_rel_rn==Inf);
    ...
    v_rel_guess_rn = damp_v*v_guess_new_rel_rn + (1-damp_v)*v_rel_old_rn;
...
for i_b = 1:N_b_g
    ...
    q_g_rel_pf(i_x,i_b) = sum(q_g(i_x,:)'.*squeeze(prob_choice_pardef(i_x,i_b,:)));
    q_g_rel_rel_pf(i_x,i_b) = sum(q_g_rel(i_x,:)'.*squeeze(prob_choice_pardef(i_x,i_b,:)));
    q_g_pf2(i_x,i_b) = sum(q_g2(i_x,:)'.*squeeze(prob_choice(i_x,i_b,:)));
    q_g_rel_pf2(i_x,i_b) = sum(q_g2(i_x,:)'.*squeeze(prob_choice_pardef(i_x,i_b,:)));
end
end
...
e_defrel = P_x*((1-def_only_pf).*(1-pardef_only_pf).*(1+(1-delta).*q_g_pf_zeros)...
   +(1-def_only_pf).*pardef_only_pf.*share.*(1+(1-delta).*q_g_rel_pf));


e_defrel2 =  P_x*((1-def_only_pf).*(1-pardef_only_pf).*(1+(1-delta).*q_g_pf2)...
   +(1-def_only_pf).*pardef_only_pf.*(1+(1-delta).*q_g_pf2));
...
for t_sim = 1:T_sim1
     ...
     if  mass_only_def(t_sim,1)>0.99 && floor((i_x_sim(t_sim,1)-1)./N_h)==(i_x_sim(t_sim,1)-1)./N_h
         prova(t_sim,1)=1;
    mass_only_def(t_sim+1,1) = sum(sum(dist_sim(:,:,t_sim).*(1-def_only_pf))); %def_pf already includes reliefs
    ...
    dist_sim(i_x_sim(t_sim+1,1),:,t_sim+1) = sum(sum(repmat(dist_sim(:,:,t_sim).*...
        (1-def_only_pf),[1,1,N_b_g]).*prob_choice,1) ,2);
    ...
    def_mean(t_sim) = sum(sum(def_new.*dist_sim(:,:,t_sim)))./sum(sum(dist_sim(:,:,t_sim)));
     ...
     elseif  mass_only_def(t_sim,1)>0.99 && floor((i_x_sim(t_sim,1)-1)./N_h)~=(i_x_sim(t_sim,1)-1)./N_h
         prova(t_sim,1)=2;
     mass_only_def(t_sim+1,1) = sum(sum(dist_sim(:,:,t_sim).*(1-def_new_pardef))); %def_pf already includes reliefs
       
    % Evolution of distribution of countries, given probabilistic decision
    % rule and exogenous income shocks
    dist_sim(i_x_sim(t_sim+1,1),:,t_sim+1) = sum(sum(repmat(dist_sim(:,:,t_sim).*...
        (1-def_new_pardef),[1,1,N_b_g]).*prob_choice_pardef,1)  ,2);
    ...
    V_g_mean_rn(t_sim) = sum(sum(v_guess_new_rel_rn.*dist_sim(:,:,t_sim)))./sum(sum(dist_sim(:,:,t_sim)));
    def_mean(t_sim) = sum(sum(def_new_pardef.*dist_sim(:,:,t_sim)))./sum(sum(dist_sim(:,:,t_sim)));
...
r_g_mean_counter(:,counter,share_counter) = r_g_mean; %Save the path of debt
q_g_mean_counter(:,counter,share_counter) = q_g_mean; %Save the path of debt
...
spread_sim2 = 10000*((1+r_g_mean2)./(1+r_vec)-1);
...
meanspread_sim(counter,1)= nanmean(spread_sim)
...
% def_sim(isnan(b_g_mean)==1 &  isnan(b_g_sim2)==0 )=1;
def_freq_sim(counter,share_counter)  = nanmean(def_sim);
...
prova3 = ((V_g_mean./V_g_mean_baseline).^(1./(1-2)))-1;
prova2_rn = (prova1_rn./prova_rn)-1;
prova3_rn = (V_g_mean_rn./V_g_mean_baseline_rn)-1;

conequivwc_V_counter(counter,share_counter)  = nanmean(nanmean(prova2));
conequivwc_V_counter1(counter,share_counter)  = nanmean(nanmean(prova3));
conequivwc_V_counter_rn(counter,share_counter)  = nanmean(nanmean(prova2_rn));
conequivwc_V_counter1_rn(counter,share_counter)  = nanmean(nanmean(prova3_rn));%%
  
clearvars -except zz jj string_num meanBY_sim meanBY_sim_market meanspread_sim medianspread_sim stdspread_sim...
    meanspread_sim2 meanspread_hurr_sim medianspread_hurr_sim counter1 counter2...
    gdp_g_h_sim spread_g_h_sim b_g_mean_counter redem_sim_counter i_x_sim_counter cons_sim spread_sim_counter...
    def_freq_sim def_hur_freq_sim hur_freq_sim gdp_mean_store mass_acc_counter V_g_mean_counter counter...
    r_g_mean_counter q_g_mean_counter share_counter share conequivwc_V_counter conequivwc_V_counter1 conequivwc_V_counter_rn...
    conequivwc_V_counter1_rn

save('climate_w_clause_persistent_pardef_wf.m')
...
save('climate_w_clause_persistent_pardef_wf.mat')
